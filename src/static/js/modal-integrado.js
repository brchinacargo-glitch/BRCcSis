/**
 * Modal de Cotação Integrado com Backend
 * Substitui o modal atual dinamicamente
 */

console.log('🔧 Carregando Modal Integrado com Backend...');

// Aguardar DOM carregar
document.addEventListener('DOMContentLoaded', function() {
    
    // Substituir modal existente
    substituirModalCotacao();
    
    // Inicializar funcionalidades
    inicializarModalIntegrado();
    
    console.log('✅ Modal Integrado - Carregado com sucesso');
});

function substituirModalCotacao() {
    const modalExistente = document.getElementById('modal-cotacao');
    if (!modalExistente) {
        console.warn('Modal existente não encontrado');
        return;
    }
    
    // Criar novo modal integrado
    const novoModal = criarModalIntegrado();
    
    // Substituir no DOM
    modalExistente.parentNode.replaceChild(novoModal, modalExistente);
    
    console.log('✅ Modal substituído com sucesso');
}

function criarModalIntegrado() {
    const modalHTML = `
...
function inicializarModalIntegrado() {
    ...
    setTimeout(() => {
        ...
        function mostrarCamposModalidade(modalidade) {
            ...
            if (camposRodoviario) camposRodoviario.style.display = 'none';
            if (camposMaritimo) camposMaritimo.style.display = 'none';
            if (camposAereo) camposAereo.style.display = 'none';
            if (camposCargaComum) camposCargaComum.style.display = 'none';
            if (camposCargaMaritima) camposCargaMaritima.style.display = 'none';
            
            // Mostrar campos específicos
            switch(modalidade) {
                case 'brcargo_rodoviario':
                    if (camposRodoviario) camposRodoviario.style.display = 'block';
                    if (camposCargaComum) camposCargaComum.style.display = 'block';
                    break;
                case 'brcargo_maritimo':
                    if (camposMaritimo) camposMaritimo.style.display = 'block';
                    if (camposCargaMaritima) camposCargaMaritima.style.display = 'block';
                    break;
                case 'frete_aereo':
        ...
        console.log('✅ Modal Integrado - Funcionalidades inicializadas');
